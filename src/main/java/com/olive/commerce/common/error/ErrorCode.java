package com.olive.commerce.common.error;

import org.springframework.http.HttpStatus;

public enum ErrorCode {
    MEMBER_NOT_FOUND(HttpStatus.NOT_FOUND),
    MEMBER_GRADE_NOT_FOUND(HttpStatus.NOT_FOUND),
    ADDRESS_NOT_FOUND(HttpStatus.NOT_FOUND),
    ADDRESS_NOT_OWNED(HttpStatus.FORBIDDEN),
    CANNOT_DELETE_ONLY_ADDRESS(HttpStatus.CONFLICT),
    PRODUCT_SOLD_OUT(HttpStatus.CONFLICT),
    COUPON_INVALID(HttpStatus.BAD_REQUEST),
    COUPON_NOT_FOUND(HttpStatus.NOT_FOUND),
    COUPON_EXPIRED(HttpStatus.BAD_REQUEST),
    COUPON_ALREADY_ISSUED(HttpStatus.CONFLICT),
    COUPON_ISSUE_LIMIT_EXCEEDED(HttpStatus.CONFLICT),
    COUPON_ALREADY_USED(HttpStatus.CONFLICT),
    INSUFFICIENT_INVENTORY(HttpStatus.CONFLICT),
    LOCK_ACQUISITION_FAILED(HttpStatus.CONFLICT),
    PAYMENT_AMOUNT_MISMATCH(HttpStatus.UNPROCESSABLE_ENTITY),
    PAYMENT_NOT_FOUND(HttpStatus.NOT_FOUND),
    IDEMPOTENCY_CONFLICT(HttpStatus.CONFLICT),
    PG_TIMEOUT(HttpStatus.GATEWAY_TIMEOUT),
    PG_FAILED(HttpStatus.BAD_GATEWAY),
    PG_WEBHOOK_INVALID(HttpStatus.UNAUTHORIZED),
    VALIDATION_FAILED(HttpStatus.BAD_REQUEST),
    AUTHENTICATION_REQUIRED(HttpStatus.UNAUTHORIZED),
    ACCESS_DENIED(HttpStatus.FORBIDDEN),
    EMAIL_ALREADY_USED(HttpStatus.CONFLICT),
    BAD_CREDENTIALS(HttpStatus.UNAUTHORIZED),
    ACCOUNT_LOCKED(HttpStatus.LOCKED),
    INVALID_REFRESH_TOKEN(HttpStatus.UNAUTHORIZED),
    BRAND_SLUG_DUPLICATE(HttpStatus.CONFLICT),
    BRAND_NOT_FOUND(HttpStatus.NOT_FOUND),
    CATEGORY_NOT_FOUND(HttpStatus.NOT_FOUND),
    CATEGORY_HAS_PRODUCTS(HttpStatus.CONFLICT),
    CATEGORY_CYCLE_DETECTED(HttpStatus.BAD_REQUEST),
    PRODUCT_NOT_FOUND(HttpStatus.NOT_FOUND),
    PRODUCT_OPTION_NOT_FOUND(HttpStatus.NOT_FOUND),
    CART_NOT_FOUND(HttpStatus.NOT_FOUND),
    CART_ITEM_NOT_FOUND(HttpStatus.NOT_FOUND),
    CART_ITEM_INVALID_OPTION(HttpStatus.BAD_REQUEST),
    INVALID_PRODUCT_STATE_TRANSITION(HttpStatus.UNPROCESSABLE_ENTITY),
    FILE_SIZE_EXCEEDED(HttpStatus.BAD_REQUEST),
    INVALID_FILE_TYPE(HttpStatus.BAD_REQUEST),
    INSUFFICIENT_POINTS(HttpStatus.UNPROCESSABLE_ENTITY),
    POINT_HISTORY_NOT_FOUND(HttpStatus.NOT_FOUND),
    ORDER_NOT_CANCELLABLE(HttpStatus.UNPROCESSABLE_ENTITY),
    ORDER_NOT_FOUND(HttpStatus.NOT_FOUND),
    ORDER_NOT_OWNED(HttpStatus.FORBIDDEN),
    REFUND_NOT_FOUND(HttpStatus.NOT_FOUND),
    INVALID_STATUS_TRANSITION(HttpStatus.UNPROCESSABLE_ENTITY),
    SEARCH_UNAVAILABLE(HttpStatus.SERVICE_UNAVAILABLE),
    INTERNAL_ERROR(HttpStatus.INTERNAL_SERVER_ERROR),
    REVIEW_ELIGIBLE_ORDER_REQUIRED(HttpStatus.UNPROCESSABLE_ENTITY),
    REVIEW_ALREADY_EXISTS(HttpStatus.CONFLICT),
    REVIEW_NOT_FOUND(HttpStatus.NOT_FOUND),
    REVIEW_NOT_OWNED(HttpStatus.FORBIDDEN),
    REVIEW_REPORT_NOT_FOUND(HttpStatus.NOT_FOUND),
    REVIEW_SELF_REPORT_NOT_ALLOWED(HttpStatus.BAD_REQUEST),
    REVIEW_ALREADY_REPORTED(HttpStatus.CONFLICT);

    private final HttpStatus httpStatus;

    ErrorCode(HttpStatus httpStatus) {
        this.httpStatus = httpStatus;
    }

    public HttpStatus httpStatus() {
        return httpStatus;
    }
}
